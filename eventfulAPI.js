...
//      console.error('Sorry there are no events based on your search. Would you like to do another search?');
//      return false;
//    } else {
...
function findEvents (keyword, callback) {
  ...
  client.searchEvents({
    ...
    if(data.search.total_items === '0'){
      console.error('Sorry there are no events based on your search.');
    } else {
      let resultEvents = data.search.events.event;
      console.log('Received ' + data.search.total_items + ' events');
      console.log('Event listings: ');
